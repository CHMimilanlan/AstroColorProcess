...
def siril_star_reduction(
    ...
    *,
    ...
    Supply either a precomputed ``starless`` image or a ``star_remover`` callback.
    When ``starless`` is supplied, it always takes priority and the callback is
    not invoked. This lets callers bypass the star-removal stage explicitly.
    ...
    starless:
        Precomputed starless image with the same shape as ``original``. When
        present, the star-removal stage is skipped completely.
    star_remover:
        Callable that receives ``original`` and returns the starless image. It
        is used only when ``starless`` is ``None``.
    star_remover_kwargs:
    ...
    if starless is not None and star_remover is not None:
        warnings.warn(
            "Both starless and star_remover were supplied. The precomputed "
            "starless image takes priority, so star_remover will not be called.",
            UserWarning,
            stacklevel=2,
        )
...
def _self_test() -> None:
    ...
    def callback_must_not_run(image: FloatImage, **kwargs: Any) -> FloatImage:
        raise AssertionError("star_remover was called despite starless being supplied.")

    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
        bypass_result = siril_star_reduction(
            original,
            starless=starless,
            star_remover=callback_must_not_run,
            value=0.2,
            output_dtype=np.float64,
        )
    np.testing.assert_allclose(bypass_result, stable, rtol=0.0, atol=0.0)
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    input_default = Path("rgb_test5.tif")
    output_default = Path("rgb_test5_star_reduced.tif")
    starless_default = Path("rgb_test5_starless.tif")

    parser = argparse.ArgumentParser(
        description="Reduce stars with the Siril DSA PixelMath-compatible algorithm."
    )
    parser.add_argument("--input", type=Path, default=input_default, help="Original stretched image.")
    parser.add_argument("--output", type=Path, default=output_default, help="Output star-reduced image.")
    parser.add_argument(
        "--starless",
        type=Path,
        default=starless_default,
        help=(
            "Precomputed starless image. When provided, the star-removal callback "
            "is skipped even if --star-remover is also specified."
        ),
    )
    parser.add_argument(
        "--star-remover",
        type=str,
        help=(
            "Custom callback: module:function or /path/file.py:function. "
            "Used only when --starless is not provided."
        ),
    )
    parser.add_argument(
        "--star-remover-kwargs",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.starless is not None:
        if args.star_remover is not None:
            warnings.warn(
                "--starless was provided; --star-remover and "
                "--star-remover-kwargs will be ignored.",
                UserWarning,
                stacklevel=2,
            )
        print(f"Using precomputed starless image: {args.starless}")
        print("Skipping the star-removal stage.")
